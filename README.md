...
Создайте локальный `.env` из примера и заполните нужные значения:
...
Пример переменных находится в `.env.example`. Backend, `npm run dev:all` и Vite
автоматически читают корневой `.env`; экспортировать значения вручную не нужно.
Уже заданные переменные окружения имеют приоритет над файлом. Основные переменные:
